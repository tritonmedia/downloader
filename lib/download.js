...
module.exports = async (config, emitter, logger) => {
    ...
    magnet: async (magnet, id, downloadPath, job) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        client.add(magnet, {
          ...
          // sadness
          let lastProgress, stallHandler, progress, lastProgressInt
          const downloadProgress = setInterval(async () => {
            ...
            if (progressInt !== lastProgressInt) {
              await global.telem.emitProgress(id, 1, progressInt)
            }
  ...
  return async job => {
    ...
    const protocol = proto.enumToString(mediaProto, 'source', media.source)
