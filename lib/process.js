 ...
 */
const findMediaFiles = async (absPath, media) => {
    ...
    klaw(absPath, {
      filter: item => {
        ...
        if (fs.statSync(item).isDirectory()) {
          // explicitly skip folders that contain the name extra
          if (/^extras/i.test(item)) {
            return false
          }

          // allow folders with "season" or s\d+
          if (/s\d+|season/i.test(item)) {
            return true
          }

          logger.warn(`skipping directory '${item}'`)
          return false
        }
...
module.exports = async (config, emitter, logger) => {
  ...
  return async job => {
    ...
    const listOfFiles = await findMediaFiles(file.path, job.media)
